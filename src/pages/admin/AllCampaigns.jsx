  ...
  //       params.endDate = dateRange[1].toISOString();
  //     }
  //     dispatch(getAllCampaignsForAdmin(params)).then((result) => {
  //       console.log('Campaign data:', result?.payload?.data?.[0]);
  //     });
  ...
  //       sheet.columns = keys.map(key => ({ header: key, key, width: key === 'User Response' || key === 'Error' ? 30 : 15 }));
  ...
  //       a.href = url;
  //       a.download = `campaign-${selectedCampaign?.CampaignName}-${new Date().toISOString().split('T')[0]}.xlsx`;
  //       a.click();
  ...
  //       const campaignsSheet = workbook.addWorksheet('Campaigns Overview');
  //       const campaignsKeys = Object.keys(campaignsData[0]);
  //       campaignsSheet.columns = campaignsKeys.map(key => ({ header: key, key, width: 20 }));
  //       campaignsData.forEach(row => campaignsSheet.addRow(row));

  //       if (allMessagesData.length > 0) {
  //         const messagesSheet = workbook.addWorksheet('All Messages');
  //         const messagesKeys = Object.keys(allMessagesData[0]);
  //         messagesSheet.columns = messagesKeys.map(key => ({ header: key, key, width: key === 'User Response' || key === 'Error' ? 30 : 20 }));
  //         allMessagesData.forEach(row => messagesSheet.addRow(row));
  //       }
  ...
  //       const a = document.createElement('a');
  //       a.href = url;
  //       a.download = `complete-campaign-report-${new Date().toISOString().split('T')[0]}.xlsx`;
  //       a.click();
  //       window.URL.revokeObjectURL(url);
  ...
  //       key: 'bot',
  //       render: (botId, record) => {
  //         console.log('Bot render:', { botId, recordBotId: record.botId, fullRecord: record });
  //         return (
  //           <Tag
  //             style={{
  //               background: '#f0f5ff',
